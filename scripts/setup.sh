...
# Verificar y actualizar/clonar repositorio frontend
echo -e "${BLUE}Checking frontend repository...${NC}"
if [ ! -d "repositories/stock-advisor-frontend" ]; then
    ...
    echo -e "${YELLOW}Frontend repository already exists. Updating...${NC}"
    cd repositories/stock-advisor-frontend
    git pull origin main
    cd ../..
    echo -e "${GREEN}Frontend repository successfully updated.${NC}"
fi

# Verificar y actualizar/clonar repositorio backend
echo -e "${BLUE}Checking backend repository...${NC}"
if [ ! -d "repositories/stock-advisor-backend" ]; then
    ...
    echo -e "${YELLOW}Backend repository already exists. Updating...${NC}"
    cd repositories/stock-advisor-backend
    git pull origin main
    cd ../..
    echo -e "${GREEN}Backend repository successfully updated.${NC}"
fi

# Verificar que el script de inicialización de base de datos existe
